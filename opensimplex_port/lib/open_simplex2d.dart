...
double noise3ImproveXY(int seed, double x, double y, double z) {
  ...
  const double ROTATE3_ORTHOGONALIZER =
      0.21132486540518713; // (Math.sqrt(3) - 1) / 2
  const double ROOT3OVER3 = 0.5773502691896258; // Math.sqrt(3) / 3
...
double noise3UnrotatedBase(int seed, double xr, double yr, double zr) {
  ...
  double value = pow(a0, 4) *
      grad(seed, xrbp + (xNMask & primeX), yrbp + (yNMask & primeY),
  ...
  value += pow(a1, 4) *
      grad(seed2, xrbp + primeX, yrbp + primeY, zrbp + primeZ, x1, y1, z1);
  ...
  if (a2 > 0) {
    ...
    if (a4 > 0) {
      ...
      double z4 = z1;
      value += pow(a4, 4) *
          grad(seed2, xrbp + (xNMask & (primeX * 2)), yrbp + primeY,
  ...
  if (a6 > 0) {
    ...
    if (a8 > 0) {
      ...
      double z8 = z1;
      value += pow(a8, 4) *
          grad(seed2, xrbp + primeX, yrbp + (yNMask & (primeY << 1)),
  ...
  if (aA > 0) {
    ...
    if (aC > 0) {
      ...
      value += pow(aC, 4) *
          grad(seed2, xrbp + primeX, yrbp + primeY,
  ...
  if (!skipD) {
    ...
    if (aD > 0) {
      ...
      double zD = z1;
      value += pow(aD, 4) *
          grad(seed2, xrbp + (xNMask & (primeX << 1)),
